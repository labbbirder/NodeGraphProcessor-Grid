...
namespace GraphProcessor
{
	...
	public class BaseGraphView : GraphView, IDisposable
	{
		...
		public SerializedProperty serializedGraph { get; private set; }
		...
		internal void UpdateExecutingState()
		{
				...
				changes.elementsToRemove.RemoveAll(e =>
				{
					...
					switch (e)
					{
						...
						case GroupView group:
							...
							RemoveElement(group);
							return true;
						case BlackboardFieldView blackboardField:
		...
		{
			evt.menu.AppendAction("Select Asset", (e) => EditorGUIUtility.PingObject(graph.unityObject), DropdownMenuAction.AlwaysEnabled);
		}
		...
		void DragPerformedCallback(DragPerformEvent e)
		{
			...
			if (dragData != null)
			{
				var exposedParameterFieldViews = dragData.OfType<BlackboardFieldView>();
		...
		void DragUpdatedCallback(DragUpdatedEvent e)
		{
			...
			if (dragData != null)
			{
				...
				if (dragData.OfType<BlackboardFieldView>().Any())
				{
					dragging = true;
				}
			}
		...
		public void Initialize(BaseGraph graph, Dictionary<string, OverlayView> overlayViews)
		{
			if (this.graph != null)
			{
				SaveGraphToDisk();
			...
			InitializeStickyNotes();
			InitializeStackNodes();
			foreach (var (id, overlayView) in overlayViews)
			{
				overlayView.InitializeGraphView(default, this);
			}
			...
			foreach (var (path, type) in NodeProvider.GetNodeMenuEntries(graph))
			{
				...
				foreach (var i in interfaces)
				{
					if (i.IsGenericType && i.GetGenericTypeDefinition() == typeof(ICreateNodeFrom<>))
					{
						...
						var initializeFunction = type.GetMethod(
						...
						if (initializeFunction.DeclaringType == type)
							nodeTypePerCreateAssetType[genericArgumentType] = (type, initializeFunction);
					}
				}
			}
		}
		...
		{
			if (graph.unityObject is IGraphOwner graphOwner)
			{
				graphOwner.CreateSerialized(out var so, out var prop);
				serializedGraph = prop;
			}
		}
		...
		/// </summary>
		/// <returns></returns>
		protected virtual BaseEdgeConnectorListener CreateEdgeConnectorListener() => new(this);
		...
		void InitializeGraphView()
		{
			...
			graph.onExecutionStateChanged -= OnExecutionStateChanged;
			graph.onExecutionStateChanged += OnExecutionStateChanged;
		...
		void InitializeEdgeViews()
		{
			...
			edge == null || edge.inputNode == null || edge.outputNode == null);
		...
		protected virtual void OnViewReloaded() { }
		...
		public bool Connect(EdgeView e, bool autoDisconnectInputs = true)
		{
			...
			if (!graph.CanConnectEdge(outputPort, inputPort))
				return false;
		...
		{
			Undo.RegisterCompleteObjectUndo(graph.unityObject, name);
		}
		...
		public void SaveGraphToDisk()
		{
			...
			if (graph.unityObject == null)
				return;

			graph.BeforeSaveToDisk();
			EditorUtility.SetDirty(graph.unityObject);
