...
namespace GraphProcessor
{
	...
	{
		const string userPortStylePath = "../res/Styles/PortViewTypes.uss";
		const string portStylePath = "../res/Styles/PortView.uss";
			...
			: base(portData.vertical ? Orientation.Vertical : Orientation.Horizontal, direction, Capacity.Multi, portData.displayType ?? fieldInfo.FieldType)
		{
			...
			styleSheets.Add(ResUtils.Load<StyleSheet>(portStylePath));

			UpdatePortSize();

			var userPortStyle = ResUtils.Load<StyleSheet>(userPortStylePath);
