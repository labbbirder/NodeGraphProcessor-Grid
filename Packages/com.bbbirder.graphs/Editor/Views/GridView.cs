...
namespace GraphProcessor
{
    public class GridView : VisualElement
    {
        ...
        private Vector2 currentTranslate;
        ...
        private void Redraw() => Redraw(true);

        private void Redraw(bool checkTransform)
        {
            if (transformSource == null || renderTexture == null || material == null)
                return;

            if (!checkTransform || (Vector2)transformSource.resolvedStyle.translate != currentTranslate
                || transformSource.resolvedStyle.scale.value.x != currentScale)
            {
                currentTranslate = (Vector2)transformSource.resolvedStyle.translate;
                currentScale = transformSource.resolvedStyle.scale.value.x;
